...
const getConfig = (support, isProd) => {
    ...
    const lessLoader = { test: /\.less$/, loader: "style!css!autoprefixer!less" };

    const fontLoader = { test: /\.(ttf|eot|svg|woff(2)?)(\?[a-z0-9=&.]+)?$/, loader: 'file-loader' };

    const copyIndexLoader = { test: /\.html$/, loader: "file?name=[name].[ext]" }; // copies the files over
